...
const UseCustomMove = () => {
  ...
  const moveToModify = useCallback(
    (tno) => { nav({ pathname: `/todo/modify/${tno}`, search: queryDefault })
  }, [nav, queryDefault]);

  const moveToRead = useCallback(
    (tno) => { nav({ pathname: `../todo/read/${tno}`, search: queryDefault });
  }, [nav, queryDefault]);

  return { moveToList, moveToModify, moveToRead, page, size, tno, nav, refresh }; //moveToModify 추가
};
